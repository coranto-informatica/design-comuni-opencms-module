			...
			<div class="container">
				<div class="row border-top border-light row-column-border row-column-menu-left">
													...
													<div id="collapse-one" class="accordion-collapse collapse show" role="region" aria-labelledby="accordion-title-one">
														<div class="accordion-body">
															<ul class="link-list" data-element="page-index">
																...
																<c:if test="${not empty value.Document}">
																	<li class="nav-item">
																		<a class="nav-link" href="#documento" data-focus-mouse="false">
																			<span><fmt:message key="label.document"/></span>
																		</a>
																	</li>
																</c:if>
																<c:if test="${not empty value.OrganizationUnit}">
																	<li class="nav-item">
																		<a class="nav-link" href="#ufficio-responsabile" data-focus-mouse="false">
																			<span><fmt:message key="label.responsibleoffice"/></span>
																		</a>
																	</li>
																</c:if>
																<c:if test="${not empty value.Attachment}">
																	<li class="nav-item">
																		<a class="nav-link" href="#formati-disponibili" data-focus-mouse="false">
																			<span><fmt:message key="label.encoding"/></span>
																		</a>
																	</li>
																</c:if>
																<c:if test="${not empty value.License}">
																	<li class="nav-item">
																		<a class="nav-link" href="#licenza-di-distribuzione" data-focus-mouse="false">
																			<span><fmt:message key="label.license"/></span>
																		</a>
																	</li>
																</c:if>
															</ul>
					...
					<section class="col-lg-9 it-page-sections-container border-light">
						<c:if test="${not empty value.Text}">
							<section id="descrizione" class="it-page-section anchor-offset" data-audio>
								<h4 class="mb-3"><fmt:message key="label.description"/></h4>
								<div ${rdfa.Text} class="richtext-wrapper lora">${value.Text}</div>
							</section>
						</c:if>
						<c:if test="${not empty value.Document}">
							<section id="documento" class="it-page-section anchor-offset mt-5" data-audio>
								<h4 class="mb-3"><fmt:message key="label.document"/></h4>
								<div class="row g-4">	
									<c:forEach var="attachmentWrapper" items="${content.valueList.Document}">
										<%
...
request.setAttribute("encodingFormat", extension);
%>
										<div class="col-12">
												...
												<div class="card card-teaser shadow rounded">
													...
													<div class="card-body">
														<h3 class="card-title h5 m-0">
															<a href="${attachmentHref}" title="${fn:escapeXml(attachmentLabel)}" aria-label="Scarica il file: ${fn:escapeXml(attachmentLabel)}" download>${attachmentLabel}</a>
														</h3>
													</div>
												</div>
											</div>
						...
						<c:if test="${not empty value.OrganizationUnit}">
							<section id="ufficio-responsabile" class="it-page-section anchor-offset mt-5" data-audio>
								<h4 class="mb-3"><fmt:message key="label.responsibleoffice"/></h4>
								<div class="row">
									<c:forEach var="organizationId" items="${content.valueList.OrganizationUnit}" varStatus="status">
										<c:set var="path">${cms.requestContext.siteRoot}/</c:set>
										<c:set var="pagesize">4</c:set>
										<c:set var="searchconfig">
											{
						...
						<c:if test="${not empty value.Document}">
							<section id="formati-disponibili" class="it-page-section anchor-offset mt-5" data-audio>
								<h4 class="mb-3"><fmt:message key="label.encoding"/></h4>
								<p class="font-serif">${encodingFormat}</p>
							</section>
						</c:if>
						<c:if test="${not empty value.License}">
							<section id="licenza-di-distribuzione" class="it-page-section anchor-offset mt-5" data-audio>
								<h4 class="mb-3"><fmt:message key="label.license"/></h4>
								<p class="font-serif">
									<c:choose>
										<c:when test="${value.License eq 'pubblicodominio'}">Pubblico dominio</c:when>
										<c:when test="${value.License eq 'attribuzione'}">Attribuzione</c:when>
										<c:when test="${value.License eq 'effettovirale'}">Effetto virale</c:when>
										<c:when test="${value.License eq 'condivisioneallostessomodocopyleftnoncompatibile'}">Condivisione allo stesso modo / copyleft non compatibile</c:when>
										<c:when test="${value.License eq 'solousononcommerciale'}">Solo uso non commerciale</c:when>
										<c:when test="${value.License eq 'nonoperederivate'}">Non opere derivate</c:when>
										<c:otherwise>Licenza sconosciuta</c:otherwise>
									</c:choose>
								</p>
							</section>
						</c:if>
