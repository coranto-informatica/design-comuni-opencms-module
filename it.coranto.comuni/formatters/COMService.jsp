...
%>
	...
	<cms:bundle basename="it.coranto.comuni.messages">
		...
		<div>
			<c:choose>
				<c:when test="${value.Category eq 'educazioneeformazione'}">
					<c:set var="categoryLabel" value="Educazione e formazione" />
				</c:when>
				<c:when test="${value.Category eq 'salutebenessereeassistenza'}">
					<c:set var="categoryLabel" value="Salute, benessere e assistenza" />
				</c:when>
				<c:when test="${value.Category eq 'vitalavorativa'}">
					<c:set var="categoryLabel" value="Vita lavorativa" />
				</c:when>
				<c:when test="${value.Category eq 'mobilitaetrasporti'}">
					<c:set var="categoryLabel" value="Mobilità e trasporti" />
				</c:when>
				<c:when test="${value.Category eq 'catastoeurbanistica'}">
					<c:set var="categoryLabel" value="Catasto e urbanistica" />
				</c:when>
				<c:when test="${value.Category eq 'anagrafestatocivile'}">
					<c:set var="categoryLabel" value="Anagrafe e stato civile" />
				</c:when>
				<c:when test="${value.Category eq 'turismo'}">
					<c:set var="categoryLabel" value="Turismo" />
				</c:when>
				<c:when test="${value.Category eq 'giustiziasicurezzapubblica'}">
					<c:set var="categoryLabel" value="Giustizia e sicurezza pubblica" />
				</c:when>
				<c:when test="${value.Category eq 'tributifinanzecontravvenzioni'}">
					<c:set var="categoryLabel" value="Tributi, finanze e contravvenzioni" />
				</c:when>
				<c:when test="${value.Category eq 'culturatempolibero'}">
					<c:set var="categoryLabel" value="Cultura e tempo libero" />
				</c:when>
				<c:when test="${value.Category eq 'ambiente'}">
					<c:set var="categoryLabel" value="Ambiente" />
				</c:when>
				<c:when test="${value.Category eq 'impreseecommercio'}">
					<c:set var="categoryLabel" value="Imprese e commercio" />
				</c:when>
				<c:when test="${value.Category eq 'autorizzazioni'}">
					<c:set var="categoryLabel" value="Autorizzazioni" />
				</c:when>
				<c:when test="${value.Category eq 'appaltipubblici'}">
					<c:set var="categoryLabel" value="Appalti pubblici" />
				</c:when>
				<c:when test="${value.Category eq 'agricolturapesca'}">
					<c:set var="categoryLabel" value="Agricoltura e pesca" />
				</c:when>
				<c:otherwise>
					<c:set var="categoryLabel" value="Tipo sconosciuto (${value.Category})" />
				</c:otherwise>
			</c:choose>
			...
			<c:forEach var="organizationId" items="${content.valueList.OrganizationUnitParent}" varStatus="status">
				...
				<c:forEach var="result" items="${search.searchResults}" varStatus="status">	
					...
					<%-- Java: Encode subito dopo il set --%>
					<%
	String venueStr = (String) pageContext.findAttribute("venue");
if (venueStr != null && !venueStr.trim().isEmpty()) {
	pageContext.setAttribute("encodedVenue", java.net.URLEncoder.encode(venueStr, "UTF-8"));
} else {
	pageContext.setAttribute("encodedVenue", "");
}
%>
				</c:forEach>

				<%-- Seconda ricerca COMVenue usando encodedVenue --%>
				<c:set var="searchconfig2">
					{
					"ignorequery": true,
					"extrasolrparams": "fq=parent-folders:\"${path}\"&fq=type:COMVenue&fq=con_locales:${cms.locale}&fq=title_${cms.locale}_s:\"${encodedVenue}\"",
					"pagesize": ${pagesize}
					}
				</c:set>	
			...
			<c:set var="whoNeedsRaw" value="${value.WhoNeeds}" />
			<c:set var="whoNeedsClean" value="${fn:replace(whoNeedsRaw, '<p>', '')}" />
			<c:set var="whoNeedsClean" value="${fn:replace(whoNeedsClean, '</p>', '')}" />
			<c:set var="whoNeedsClean" value="${fn:replace(whoNeedsClean, '<br />', '')}" />
			...
			<script type="application/ld+json" data-element="metatag">
 {
		  ...
		  "serviceOperator": {
            ...
            "name": "<fmt:message key="label.townname"/>"
          },
          "areaServed": {
          ...
          "audience": {
            "@type": "Audience",
            "audienceType": "${whoNeedsClean}"
          },
          "availableChannel": {
            "@type": "ServiceChannel",
            "name": "Dove rivolgersi",
            "serviceUrl": "${value.SubmitRequestOnline.value.RequestLink.value.Link}",
            "availableLanguage": {
              "@type": "Language",
              "name": "Italian",
              "alternateName": "it"
            ...
            "serviceLocation": {
              ...
              "address": {
                ...
                "addressLocality": "<fmt:message key="label.townname"/>",
              }
            }
          }
        }
													...
													<div id="collapse-one" class="accordion-collapse collapse show" role="region" aria-labelledby="accordion-title-one">
														<div class="accordion-body">
															<ul class="link-list" data-element="page-index">											
																...
																<c:if test="${(not empty value.SubmitRequestOnline) or (not empty value.SubmitRequestOffline)}">
																	<li class="nav-item">
																		<a class="nav-link" href="#submit-request" data-focus-mouse="false">
																			<span><fmt:message key="label.submitrequest"/></span>
																		</a>
																	</li>
																</c:if>
																<c:if test="${not empty value.FurtherInformation}">
																	<li class="nav-item">
																		<a class="nav-link" href="#further-informations" data-focus-mouse="false">
						...
						<div class="it-page-sections-container">
							...
							<c:if test="${(not empty value.SubmitRequestOnline) or (not empty value.SubmitRequestOffline)}">
								<section class="it-page-section mb-30 has-bg-grey p-4">
									<h2 class="mb-3" id="submit-request"><fmt:message key="label.submitrequest"/></h2>

									<!-- SubmitRequestOnline -->
									<c:if test="${not empty value.SubmitRequestOnline.value.RequestLink}">
										<p ${value.SubmitRequestOnline.rdfa.Information} class="text-paragraph lora mb-4" data-element="service-generic-access">
											${value.SubmitRequestOnline.value.Information}
										</p>
										<button type="button" class="btn btn-primary mobile-full"
												onclick="window.open('${value.SubmitRequestOnline.value.RequestLink.value.Link}', '${value.SubmitRequestOnline.value.RequestLink.value.LinkTarget}');"
												data-element="service-online-access" data-focus-mouse="false">
											<span>${value.SubmitRequestOnline.value.RequestLink.value.LinkTitle}</span>
										</button>
									</c:if>

									<!-- SubmitRequestOffline -->
									<c:if test="${not empty value.SubmitRequestOffline.value.AppointmentLink}">
										<p ${value.SubmitRequestOffline.rdfa.Information} class="text-paragraph lora mb-4" data-element="service-generic-access">
											${value.SubmitRequestOffline.value.Information}
										</p>
										<button type="button" class="btn btn-outline-primary t-primary bg-white mobile-full"
												onclick="window.open('${value.SubmitRequestOffline.value.AppointmentLink.value.Link}', '${value.SubmitRequestOffline.value.AppointmentLink.value.LinkTarget}');"
												data-element="service-booking-access" data-focus-mouse="false">
											<span>${value.SubmitRequestOffline.value.AppointmentLink.value.LinkTitle}</span>
										</button>
									</c:if>
								</section>
							</c:if>
							...
							<c:if test="${not empty value.FurtherInformation}">
								<section class="it-page-section mb-30">
									<h2 class="title-xxlarge mb-3" id="further-informations"><fmt:message key="label.furtherinformations"/></h2>
									<div ${rdfa.FurtherInformation} class="richtext-wrapper lora">
...
%>
							...
							<c:if test="${not empty value.Otherdoc}">
								<section class="it-page-section mb-30">
									<h2 class="title-xxlarge mb-3" id="documents"><fmt:message key="label.documents"/></h2>
																	...
																	<c:forEach var="result" items="${search.searchResults}" varStatus="innerStatus">
																		...
																		<p class="u-main-black">												
																			...
																			<c:if test="${not empty email}">
																				Email: <a href="<cms:link>mailto:${email}</cms:link>" aria-label="Invia una Email a ${email}" title="Invia una Email a ${email}" target="_blank">${email}</a>
																			</c:if>
																			<c:if test="${not empty fax}">
																				Fax: ${fax}
																			</c:if>
																			<c:if test="${not empty pec}">
																				PEC: <a href="<cms:link>mailto:${pec}</cms:link>" aria-label="Invia una PEC a ${pec}" title="Invia una PEC a ${pec}" target="_blank">${pec}</a>
																			</c:if>
																			<c:if test="${not empty accountType and not empty link}">
																				<c:choose>
																					<c:when test="${accountType eq 'whatsapp'}">
																						<a href="${link}" aria-label="Invia un messaggio su WhatsApp" title="Invia un messaggio su WhatsApp" target="_blank">WhatsApp</a>
																					</c:when>
																					<c:when test="${accountType eq 'telegram'}">
																						<a href="${link}" aria-label="Invia un messaggio su Telegram" title="Invia un messaggio su Telegram" target="_blank">Telegram</a>
																					</c:when>
																					<c:when test="${accountType eq 'skype'}">
																						<a href="${link}" aria-label="Avvia una conversazione su Skype" title="Avvia una conversazione su Skype" target="_blank">Skype</a>
																					</c:when>
																					<c:when test="${accountType eq 'linkedin'}">
																						<a href="${link}" aria-label="Visita il profilo LinkedIn" title="Visita il profilo LinkedIn" target="_blank">Linkedin</a>
																					</c:when>
																					<c:when test="${accountType eq 'twitter'}">
																						<a href="${link}" aria-label="Visita il profilo Twitter" title="Visita il profilo Twitter" target="_blank">Twitter</a>
																					</c:when>
																					<c:otherwise>
																						${accountType}:
																						<a href="${link}" aria-label="Vai a ${accountType}" title="Vai a ${accountType}" target="_blank"> ${link}</a>
																					</c:otherwise>
																				</c:choose>
																			</c:if>
																			<c:if test="${not empty url}">
																				Sito web: <a href="${url}" aria-label="Vai alla pagina: ${url}" title="Vai alla pagina: ${fn:escapeXml(Url)}" target="_blank">${url}</a>
																			</c:if>	
																		</p>
																	</c:forEach>															
																</c:forEach>	
